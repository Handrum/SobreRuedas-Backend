...
export class EstadoSolicitudController {
  ...
  async findById(
    ...
    @param.filter(EstadoSolicitud, {exclude: 'where'})
    filter?: FilterExcludingWhere<EstadoSolicitud>,
  ): Promise<EstadoSolicitud> {
    return this.estadoSolicitudRepository.findById(id, filter);
  }

  @patch('/Estado_Solicitudes/{id}')
  @response(204, {
